        ...
        if(((scause<<1)>>1)==8) 
        {
            ...
            switch (regs->x[17])    // a7
            {
                case SYS_READ:
                    ...
                    break;
                case SYS_WRITE:
                    // regs->x[10]=sys_write(regs->x[10],(const char*)regs->x[11],(size_t)regs->x[12],regs); // a0,a1,a2
                    regs->x[10] = sys_write(regs->x[10], (const char *)regs->x[11], regs->x[12]);   // lab6 fs added
                    break;
                case SYS_GETPID:
                    regs->x[10]=sys_getpid(regs);
                    break;
                case SYS_CLONE:
                    regs->x[10]=do_fork(regs);
                default:
                    Err("not support syscall id = %d", regs->x[17]);
                    break;
            }
