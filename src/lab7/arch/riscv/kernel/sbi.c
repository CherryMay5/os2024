...
// eid 和 console_write_byte 一样为 0x4442434e，fid 为 1
// 参数 num_bytes 为读取的字节数，base_addr_lo 和 base_addr_hi 为写入的目的地址
struct sbiret sbi_debug_console_read(uint64_t num_bytes, uint64_t base_addr_lo, uint64_t base_addr_hi)
{
    struct sbiret result=sbi_ecall(SBI_EID_DEBUG_CONSOLE_WRITE_BYTE,1,num_bytes,base_addr_lo,base_addr_hi,0,0,0);
    return result;
}